...
-- 4 Marks
SELECT COALESCE(department_id, 'X') AS department_id,
       SUM(salary) AS total_salary_expense
FROM employees
GROUP BY COALESCE(department_id, 'X')
ORDER BY COALESCE(department_id, 'X');
...
where year(job_history.start_date) > 2000
		and employees.salary > 5000 
        and employees.first_name like '%A%';
...
-- 5 Marks
SELECT manager_id, SUM(salary) AS total_salary_expense
FROM employees
GROUP BY manager_id;
...
join departments on employees.department_id = departments.department_id
where salary > 10000
group by departments.department_name,employees.first_name,employees.last_name;
...
select employees.first_name,employees.last_name
from employees
join job_history on employees.employee_id = employees.employee_id
where year(job_history.start_date) > year(job_history.end_date);
...
-- 8 Marks
select departments.department_name,sum(employees.salary) as "Salary Expense",
(SUM(employees.salary) / (SELECT SUM(employees.salary) FROM employees)) * 100 AS percentage_contribution
from employees
join departments on employees.department_id = departments.department_id
group by departments.department_name;
